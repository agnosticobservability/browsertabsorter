import { applyGrouping, applySorting, fetchState, ICONS, mapWindows, sendMessage } from "./common.js";
// Elements
...
const renderTree = () => {
    ...
    filtered.forEach(({ window, visibleTabs }) => {
        ...
        if (!expandedNodes.has(windowKey) && !query)
        ...
        Array.from(groups.entries()).sort().forEach(([groupLabel, groupData]) => {
            ...
            if (groupData.color)
                grpContent.style.color = groupData.color;
...
const loadState = async () => {
    ...
    windowState.forEach(w => {
        if (!expandedNodes.has(`w-${w.id}`))
            expandedNodes.add(`w-${w.id}`);
    });
    renderTree();
};
