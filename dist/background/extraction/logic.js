...
export function normalizeUrl(urlStr) {
    try {
        ...
        const keep = [];
        if (isYoutube)
            keep.push('v', 'list', 't', 'c', 'channel', 'playlist');
        if (isGoogle)
            keep.push('q', 'id', 'sourceid');
...
export function parseYouTubeUrl(urlStr) {
    try {
        const url = new URL(urlStr);
...
export function extractJsonLdFields(jsonLd) {
    let author = null;
    ...
    if (mainEntity) {
        if (mainEntity.author) {
            ...
            else if (Array.isArray(mainEntity.author) && mainEntity.author[0]?.name)
                author = mainEntity.author[0].name;
        }
        if (mainEntity.datePublished)
            publishedAt = mainEntity.datePublished;
        if (mainEntity.dateModified)
            modifiedAt = mainEntity.dateModified;
        if (mainEntity.keywords) {
            if (typeof mainEntity.keywords === 'string')
                tags = mainEntity.keywords.split(',').map((s) => s.trim());
            else if (Array.isArray(mainEntity.keywords))
