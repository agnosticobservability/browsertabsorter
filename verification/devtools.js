...
import { groupTabs, domainFromUrl, semanticBucket, navigationKey, groupingKey } from "../background/groupingStrategies.js";
import { GENERA_REGISTRY } from "../background/extraction/generaRegistry.js";
import { sortTabs, recencyScore, hierarchyScore, pinnedScore, compareBy } from "../background/sortingStrategies.js";
...
function initStrategyBuilder() {
    ...
    if (addFilterBtn)
        addFilterBtn.addEventListener('click', () => addBuilderRow('filter'));
    if (addGroupBtn)
        addGroupBtn.addEventListener('click', () => addBuilderRow('group'));
    if (addSortBtn)
        addSortBtn.addEventListener('click', () => addBuilderRow('sort'));
    if (saveBtn)
        saveBtn.addEventListener('click', saveCustomStrategyFromBuilder);
    if (runBtn)
...
function addBuilderRow(type, data) {
    let containerId = '';
    if (type === 'filter')
        containerId = 'filter-rows-container';
    else if (type === 'group')
    ...
    if (data) {
        if (type === 'filter') {
            ...
            if (data.operator)
                div.querySelector('.operator-select').value = data.operator;
            if (data.value)
                div.querySelector('.value-input').value = data.value;
        }
        else if (type === 'group') {
            const sourceSelect = div.querySelector('.source-select');
            const fieldSelect = div.querySelector('.value-input-field');
...
function getBuilderStrategy() {
    const idInput = document.getElementById('strat-id');
    const labelInput = document.getElementById('strat-label');
    const fallbackInput = document.getElementById('strat-fallback');
    const id = idInput.value.trim();
    const label = labelInput.value.trim();
    const fallback = fallbackInput.value.trim();
    const sortGroups = document.getElementById('strat-sortgroups').checked;
    if (!id || !label) {
        return null;
    }
    const filters = [];
...
function runBuilderSimulation() {
    ...
    if (!strat) {
        if (!document.getElementById('strat-id').value) {
    ...
    container.querySelectorAll('.edit-strat-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (strat) {
                ...
                document.getElementById('strat-label').value = strat.label;
                document.getElementById('strat-fallback').value = strat.fallback || '';
                document.getElementById('strat-sortgroups').checked = !!strat.sortGroups;
                // Clear lists
