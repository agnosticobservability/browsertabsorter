import { applyTabGroups, applyTabSorting, calculateTabGroups, fetchCurrentTabGroups, mergeTabs, unmergeTabs } from "./tabManager.js";
...
const handleMessage = async <TData>(
  ...
  switch (message.type) {
    ...
    case "unmergeSelection": {
      ...
      const payload = message.payload as { tabIds: number[] };
      if (payload?.tabIds?.length) {
        await unmergeTabs(payload.tabIds);
        return { ok: true };
      }
      return { ok: false, error: "No tabs selected" };
    }
