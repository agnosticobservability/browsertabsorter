...
const analyzeYoutubeContext = async (tab: TabMetadata): Promise<ContextResult | null> => {
  ...
  try {
    const results = await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        const title = document.querySelector<HTMLElement>("h1.ytd-video-primary-info-renderer")?.innerText
            || document.querySelector<HTMLElement>("#title h1")?.innerText
            || document.title;

        const channel = document.querySelector<HTMLElement>("#upload-info #channel-name a")?.innerText
            || document.querySelector<HTMLElement>("ytd-channel-name a")?.innerText
            || document.querySelector<HTMLElement>(".ytd-channel-name a")?.innerText
            || "";

        const date = document.querySelector<HTMLElement>("#info-strings yt-formatted-string")?.innerText
            || document.querySelector<HTMLElement>("#date yt-formatted-string")?.innerText
            || "";
...
const localHeuristic = async (tab: TabMetadata): Promise<ContextResult> => {
  ...
  else if (url.includes("netflix") || url.includes("spotify") || url.includes("hulu") || url.includes("disney")) context = "Entertainment";
