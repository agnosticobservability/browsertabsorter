...
const resolveWindowMode = (modes) => {
    if (modes.includes("new"))
        return "new";
    if (modes.includes("compound"))
        return "compound";
    return "current";
};
    ...
    tabs.forEach((tab) => {
        ...
        try {
            for (const s of effectiveStrategies) {
                const result = (0, exports.getGroupingResult)(tab, s);
                if (result.key !== null) {
...
const getGroupingResult = (tab, strategy) => {
    const custom = customStrategies.find(s => s.id === strategy);
    if (custom) {
        ...
        if (groupingRulesList.length > 0) {
            ...
            if (result)
                return { key: result, mode: "current" };
        }
        return { key: custom.fallback || "Misc", mode: "current" };
    }
    // Built-in strategies
    ...
    switch (strategy) {
        case "domain":
        case "domain_full":
            simpleKey = (0, exports.domainFromUrl)(tab.url);
            break;
        case "topic":
            simpleKey = (0, exports.semanticBucket)(tab.title, tab.url);
            break;
        case "lineage":
            simpleKey = (0, exports.navigationKey)(tab);
            break;
        case "context":
...
exports.getGroupingResult = getGroupingResult;
const groupingKey = (tab, strategy) => {
    return (0, exports.getGroupingResult)(tab, strategy).key;
};
